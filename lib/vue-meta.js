...
function _inject (options) {
  ...
  return function inject () {
    ...
    // Here the title is already created
    // Thus we'll replace it with proper title
    const title = info.title
    const ogTitle = info.meta.find(meta => meta.property === 'og:title')
    if (ogTitle) {
      ogTitle.content = title
    } else {
      info.meta.push({ property: 'og:title', content: title })
    }

    const twitterTitle = info.meta.find(meta => meta.name === 'twitter:title')
    if (twitterTitle) {
      twitterTitle.content = title
    } else {
      info.meta.push({ name: 'twitter:title', content: title })
    }
    ...
    if (description) {
      // Here the title is already created
      // Thus we'll replace it with proper title
      const ogDesc = info.meta.find(meta => meta.property === 'og:description')
      if (ogDesc) {
      ...
      const twitterDesc = info.meta.find(meta => meta.name === 'twitter:description')
      if (twitterDesc) {
        twitterDesc.content = description.content
      } else {
    ...
    if (image) {
      // Here the image is already created
      // Thus we'll replace it with proper image
      const ogImage = info.meta.find(meta => meta.property === 'og:image')
      if (ogImage) {
        ogImage.content = image.content
      } else {
        info.meta.push({ property: 'og:image', content: image.content })
      }

      const twitterImage = info.meta.find(meta => meta.name === 'twitter:image')
      if (twitterImage) {
        twitterImage.content = image.content
      } else {
